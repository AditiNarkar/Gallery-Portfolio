...
import wall from "../app/assets/wall2.jpeg"
import mobileWall from "../app/assets/wall_vertical.jpeg"
    ...
    return (
        <>
            <div className={`${isMobile ? 'h-screen' : ''}`}>
                {!isMobile && <Image className={`${styles.floor}`} src={floor} />}
                <Image
                    className={`${styles.wall} transition-transform duration-500 w-full object-cover`}
                    src={isMobile ? mobileWall : wall}
                />
            </div>
